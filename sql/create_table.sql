...
-- 被拦截 prompt 记录表
create table if not exists prompt_block_log
(
    id            bigint auto_increment comment 'id' primary key,
    promptContent text                               not null comment '被拦截的 prompt 内容',
    blockReason   varchar(512)                       not null comment '拦截原因',
    appId         bigint                             not null comment '应用id',
...
) comment '被拦截 prompt 记录' collate = utf8mb4_unicode_ci;
